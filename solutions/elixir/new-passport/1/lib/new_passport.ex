defmodule NewPassport do
  def get_new_passport(now, birthday, form) do
    with {:ok, datetime} <- enter_building(now),
          {:ok, manual} <- find_counter_information(now),
          counter <- manual.(birthday),
          {:ok, checksum } <- stamp_form(datetime, counter, form),
          passport_number <- get_new_passport_number(datetime, counter, checksum) do
      {:ok, passport_number}
    else
      {:coffee_break, instructions} -> {:retry, NaiveDateTime.add(now, 15 * 60)}
